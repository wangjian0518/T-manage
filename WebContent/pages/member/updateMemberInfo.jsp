...
+'<td><select class="easyui-combobox" id="memberStatus" name="memberStatus" ><option value="0">可用</option><option value="1">不可用</option></select></td>';
			...
			text += '</tr>';
			if(paramsobj!=null){
			text += '<input type="hidden" id="memberId" name="memberId"/>';
			}
			...
			if(paramsobj!=null){
			$('#memberId').val(paramsobj.memberId);
			}
			
			$('#memberName').val(paramsobj==null?null:paramsobj.memberName);
			$('#memberStatus').val(paramsobj==null?null:paramsobj.memberStatus);
