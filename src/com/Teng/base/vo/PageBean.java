...
public class PageBean  implements Serializable{
	...
	public PageBean(int currentlyPage, int pageSize, Integer recordsCount) {
		...
		this.countResults = recordsCount;
		...
		} else if (currentlyPage > this.totalPage) {
			this.currentlyPage = totalPage;
		} else {
			this.currentlyPage = currentlyPage;
		}
		this.start = (this.currentlyPage - 1) * this.pageSize + 1;
		this.end = this.currentlyPage * this.pageSize;
		if (this.currentlyPage == 1)
			this.upPage = false;
		if (this.currentlyPage == this.totalPage)
			this.nextPage = false;
